...
.controller('IndividualChartController',[ '$stateParams' ,'$scope', 'ChartStore' , 'PieChart', 'BarChart', 'TimeSeriesChart', 'DonutChart', 'StackedChart',
    function($stateParams,$scope,ChartStore,PieChart,BarChart,TimeSeriesChart,DonutChart,StackedChart){
        ...
        var chart_data = ChartStore.getChart( $stateParams.chartId );
        if( ! chart_data){
            $scope.title = "Could not find chart -> " + $stateParams.chartId;
            return;
        }
        $scope.kind = chart_data.kind;

        var myChart;
        if( chart_data.type == 'peopleByDepartmentPerLocation'){
            myChart = new StackedChart(chart_data);
        }
        else if( $scope.kind == 'bar'){
            myChart = new BarChart(chart_data);
        }
        else if($scope.kind == 'pie'){
            myChart = new PieChart(chart_data);
        }
        else if($scope.kind == 'donut'){
            myChart = new DonutChart(chart_data);
        }
        else if($scope.kind == 'line'){
            myChart = new TimeSeriesChart(chart_data);
        }
        else{
        ...
        $scope.chartLabel = chart_data.seriesTitle;
    }]);
